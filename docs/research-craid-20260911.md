...
Status: **working capture**. Not a preprint. Not a schema change. Claims that need measurement stay unlabeled.

This is the Continuous Research pass that the [C(RAID)](../spec/craid.md) hybrid already names. Analysis / Integration / Deployment are later. The machine contract is still `schema/` + `tests/validate.py`. Human contract: [`working-note.md`](working-note.md).

Search method (this pass): DuckDuckGo via `ddgs` (native `duckduckgo` backend returned empty from this IP; `auto`/`brave` metasearch used), agent-reach Jina Reader (`curl https://r.jina.ai/URL`), GitHub `gh`, arXiv `id_list`. Tavily was down. 50 alias queries, 398 hits, 58 arXiv papers pulled by id.

Frontier-kb landing: `inbox/cursor/` notes on [kvnloo/frontier-kb](https://github.com/kvnloo/frontier-kb) (CoS promotes; this tree does not vendor the vault).
...
| [2603.06007](https://arxiv.org/abs/2603.06007) MASFactory | graph IR + Vibe Graphing (NL $\to$ editable spec $\to$ executable). Control / message / state flows. HITL Interaction node. | already AODL’s three-object + data/control split; visualizer is $\tau$, not the language | treating NL-compiled graphs as observed $\mathcal{O}_t$ |
...
| [2512.24601](https://arxiv.org/abs/2512.24601) RLM | prompt lives in the environment; recursive self-calls over slices | token-slice hypothesis in the 0.2 spec; `memory` node is not the context window | claiming RLM proves dynamic-graph interchange |
...
| OpenAI [harness engineering](https://openai.com/index/harness-engineering/) | humans specify intent + feedback loops; repo is system of record; AGENTS.md is a TOC; mechanical invariants | observed $V$ vs intent; compile AODL $\to$ repo laws, not the reverse | 0-lines-human as a requirement here |
...
Do not put Grothendieck topologies in JSON. Put the reading here so Solarpunk does not invent a fourth object called “the field.”
...
7. **Sheaf reading in the catalog UI.** Explain intent/plan/observed as non-glueable until receipts exist. Do not add sheaf fields.
8. **C(RAID) stays the named hybrid** for this factory. New hybrids need `policies.kinds` + fixtures, same as today.

## 6. Work with frontier-kb

Notes from this pass go to frontier-kb `inbox/cursor/` (distributed-writer rule). CoS promotes literature/permanent. Do not copy this file into Dash. Do not treat frontier-kb as a runtime. Postgres CAS is the operational store; this markdown is the review export.

## 7. Non-goals (reaffirmed)

No arXiv. No `.tex`. No second scheduler. No payment. No inferred swarm. No mixing Dash UI into this tree. No claiming $\lambda_A$ or Pact as AODL 0.3.

Proof that this file did not change the language: `python3 tests/validate.py`.
