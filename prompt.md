...
| | |
|---|---|
| **This repo** | https://github.com/kvnloo/aodl |
| Clone | `git clone https://github.com/kvnloo/aodl.git` |
...
1. Clone or open https://github.com/kvnloo/aodl. `cd` into that tree. `git fetch origin`. `python3 .verified-oss-loop/rollout.py show`. Branch from `origin/$(python3 .verified-oss-loop/rollout.py get worker_base)` unless the issue names another base. Day-pass PRs target `feature_target`. Overnight PRs target `overnight_target`.
...
```bash
gh issue list --repo kvnloo/aodl --label claimable --state open
gh pr list --repo kvnloo/aodl --state open
```
