"""HOTL/AODL validator core: fail-closed document checks, importable.

Zero dependencies. This is the same validation logic the `tests/validate.py`
CLI runs; it was extracted verbatim so a realtime gate can call it in-process
without spawning a subprocess or parsing CLI output.

This module validates DOCUMENTS only. It is not a scheduler, runtime,
payment system, or event loop.

Gate entry point:

    from validator import validate_document
    errors = validate_document(doc)   # [] means valid; non-empty means deny
"""
...
def validate_document(doc: dict[str, object]) -> list[str]:
    """Validate a HOTL/AODL document for a live orchestration gate.

    Returns a list of error strings; an empty list means the document is
    valid. Fail closed: unknown versions, unbounded spawn, implicit fan-in,
    dependency cycles, undeclared privileged capabilities, and payment
    execution grants all produce errors.
    """
    return [str(issue) for issue in validate(doc)]
